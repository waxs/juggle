...
#### select(set, unique)
The select method retrieves a set from the Juggle store. The select method can also be used to retrieve any nested values, for instance `juggle.select('name.age')`. Using the second argument you can pass a boolean for retrieving each value or return an array of unique values (by default this will be true). If a given set is unavailable it will return `null`.
